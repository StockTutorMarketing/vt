...
export default {
    ...
    getBars: async (symbolInfo, resolution, periodParams, onHistoryCallback, onErrorCallback) => {
        ...
        try {
            ...
            const mappedResolution = resolutionMapping[resolution] || 'minute';
    
            // Fetch the data
            let data = await makeApiRequest(from, to, instrumentKey, mappedResolution, countBack);
            let candles = data;
    
            console.log('Requested from:', new Date(from * 1000).toISOString());
            console.log('Requested to:', new Date(to * 1000).toISOString());
            console.log(candles.length, "candles fetched");
    
            // If fewer candles than requested are fetched, expand the time range
            if (candles.length < countBack) {
                let retryAttempts = 0;
                let newFrom = from;
    
                while (candles.length < countBack && retryAttempts < 3) {  // Limit the number of retries to prevent infinite loops
                    // Expand the `from` date further back
                    newFrom = newFrom - (countBack * 60);  // Extend time range (countBack * 1-minute intervals)
    
                    console.log(`Expanding from date to: ${new Date(newFrom * 1000).toISOString()}`);
                    
                    // Fetch more data with the expanded range
                    const moreData = await makeApiRequest(newFrom, to, instrumentKey, mappedResolution, countBack);
                    candles = [...moreData, ...candles];  // Merge old and new data
    
                    retryAttempts++;
                }
            }
    
            // Convert the fetched candles into TradingView format
            let bars = [];
            candles.forEach(candle => {
                const candleTime = candle[0] * 1000;  // Convert to milliseconds
                bars.push({
                    time: candleTime,  // Ensure time is in milliseconds
                    open: candle[1],
                    high: candle[2],
                    low: candle[3],
                    close: candle[4],
            ...
            console.log(bars.length, "bars prepared for TradingView");
    
            // Provide bars to TradingView
